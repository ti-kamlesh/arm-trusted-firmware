...
struct ti_lpsc_module_data {
	 ...
	 */
	uint8_t depends_psc_idx;	/* ti_psc_idx_t */
	 ...
	 */
	uint8_t powerdomain;		/* ti_pd_idx_t */
};
...
#define TI_PSC_PD(id)[(id)] = { .flags = TI_PSC_PD_EXISTS }

/** Declare an always on PSC powerdomain entry */
#define TI_PSC_PD_AON(id)[(id)] = { .flags = TI_PSC_PD_ALWAYSON | TI_PSC_PD_EXISTS }

/** Declare a PSC powerdomain with a dependency */
#define TI_PSC_PD_DEP(id, dep)[(id)] = { .depends = (dep),		\
 ...
 *
 * \return
 * The ti_pd_idx_t index of the power domain within the PSC.
 */
 ...
 *
 * \return
 * The ti_lpsc_idx_t index of the module within the PSC.
 */
 ...
 *
 * \param id
 * The ti_psc_idx_t index of the PSC device to find.
 *
 * \return
