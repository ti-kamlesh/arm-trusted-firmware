...
/**
 * soc_device_get_state_internal() - Get the PSC state for a single SoC device domain.
...
static uint32_t soc_device_get_state_internal(const struct ti_soc_device_data *dev)
{
	...
	if (psc_dev != NULL) {
		...
		if (module != NULL) {
			ret = ti_lpsc_module_get_state(psc_dev, module);
		}
	}
...
/**
 * soc_device_set_reset_iso_internal() - Set reset isolation for a single SoC device domain.
 * @dev: SoC device data identifying the PSC index and LPSC module.
...
/**
 * soc_device_get_context_loss_count_internal() - Get context loss count for a device domain.
 * @dev: SoC device data identifying the PSC index and LPSC module.
 *
...
/**
 * soc_device_enable_internal() - Enable the PSC LPSC module for a single SoC device domain.
 * @dev: SoC device data identifying the PSC index and LPSC module.
 */
...
/**
 * soc_device_disable_internal() - Disable the PSC LPSC module for a single SoC device domain.
 * @dev: SoC device data identifying the PSC index and LPSC module.
 */
...
/**
 * soc_device_disable_internal_flags_iterate() - Clear all PSC initialization flags.
...
/**
 * soc_device_disable_internal_flags() - Clear PSC initialization flags for a device.
