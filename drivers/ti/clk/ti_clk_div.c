...
uint32_t ti_clk_get_div(struct ti_clk *clkp)
{
	...
	divp = container_of(clkp->drv, const struct ti_clk_drv_div, drv);
					    ...
					    bool *changed)
{
	...
	*changed = false;
	...
	data_div = container_of(clkp->data, const struct ti_clk_data_div, data);
			  ...
			  bool *changed)
{
	...
	*changed = false;
	...
	if (!p || !ti_clk_lookup((ti_clk_idx_t) p->clk)) {
...
bool ti_clk_div_reg_set_div(struct ti_clk *clkp, uint32_t div)
{
	...
	if ((d_val_p <= n)
		...
		reg_val &= ~(TI_MASK_COVER_FOR_NUMBER(n) << data_reg->bit);
