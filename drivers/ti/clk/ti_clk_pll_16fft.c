...
static bool clk_pll_16fft_wait_for_lock(struct ti_clk *clock_ptr)
{
	...
	data_pll = container_of(clock_ptr->data, const struct ti_clk_data_pll,
				data);
				       ...
				       uint32_t clkod)
{
	...
	if (div_clk != NULL) {
		if (div_clk->drv != NULL) {
			drv_div = container_of(div_clk->drv,
						...
						bool *changed)
{
	...
	data_pll = container_of(pll_clk->data, const struct ti_clk_data_pll,
				data);
	pll = container_of(data_pll, const struct ti_clk_data_pll_16fft,
			   data_pll);
							       ...
							       bool *changed)
{
	...
	data_pll = container_of(pll_clk->data, const struct ti_clk_data_pll,
				data);
	pll = container_of(data_pll, const struct ti_clk_data_pll_16fft, data_pll);
	data_div = container_of(pll_clk->data, const struct ti_clk_data_div,
				data);
	...
	for (i = 0; data_pll->pll_entries[i] != TI_PLL_TABLE_LAST; i++) {
		/*
...
static uint32_t clk_pll_16fft_get_freq(struct ti_clk *clock_ptr)
{
	...
	if (clock_ptr->ref_count == 0U) {
		...
		data_pll = container_of(clock_ptr->data,
...
static bool clk_pll_16fft_postdiv_set_div(struct ti_clk *clock_ptr, uint32_t div)
{
	...
	if (pll_clk != NULL) {
		...
		data_pll = container_of(pll_clk->data,
					const struct ti_clk_data_pll, data);
		pll = container_of(data_pll, const struct ti_clk_data_pll_16fft,
				   data_pll);
...
static int32_t clk_pll_16fft_hsdiv_init(struct ti_clk *clkp)
{
	...
	data_div = container_of(clkp->data, const struct ti_clk_data_div,
				data);
	data_reg = container_of(data_div, const struct ti_clk_data_div_reg,
				data_div);
	drv_div = container_of(clkp->drv, const struct ti_clk_drv_div, drv);
