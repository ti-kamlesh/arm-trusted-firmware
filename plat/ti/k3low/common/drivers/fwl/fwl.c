...
void update_fwl_configs(void)
{
	 ...
	 * Foreground regions override the permissions set by a background
	 * region; foreground regions cannot be configured without a background
	 * region already configured for a given firewall for an address range.
	 * All foreground regions must therefore be disabled first before disabling
	 * background regions.
	 */
