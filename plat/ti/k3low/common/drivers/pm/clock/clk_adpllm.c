 ...
 */
#define ADPLLM_FREQ_CTRL1_DCO_SHIFT	24U
#define ADPLLM_FREQ_CTRL1_DCO_MASK	(((uint32_t) 7U) << 24U)
#define ADPLLM_FREQ_CTRL1_DCO_1500MHZ	(((uint32_t) 2U) << 24U)
#define ADPLLM_FREQ_CTRL1_DCO_2500MHZ	(((uint32_t) 4U) << 24U)
/*
 ...
 */
#define ADPLLM_SS_SPREAD_DELTA_SHIFT	24U
 ...
 */
static bool adpllm_dcc_pllm_valid(struct clk *clkp __unused, uint32_t pllm,
				  bool is_frac)
{
	bool ret = true;
	bool m4x;
...
static int32_t adpllm_bin(struct clk *clkp __unused, uint32_t plld __unused,
			  uint32_t pllm __unused, bool do_frac,
...
static uint32_t adpllm_vco_fitness(struct clk *clkp __unused, uint32_t vco,
				   bool do_frac __unused)
{
	return vco;
}
 ...
 * \return true if pllm value is valid, false otherwise
 */
static bool adpllm_pllm_valid(struct clk *clkp __unused, uint32_t pllm,
			      bool is_frac)
{
	bool ret = true;
	bool m4x;
...
static bool clk_adpllm_check_lock(struct clk *clkp)
{
	...
	data_pll = (const struct clk_data_pll *)clk_datap->data;
...
static void clk_adpllm_program_freq(struct adpllm_program_data *data)
{
	...
	if (data->pll->ljm) {
		...
		freq_ctrl1 &= ~ADPLLM_FREQ_CTRL1_DCO_MASK;
		if (dco < cutoff) {
			freq_ctrl1 |= ADPLLM_FREQ_CTRL1_DCO_1500MHZ;
		} else {
			freq_ctrl1 |= ADPLLM_FREQ_CTRL1_DCO_2500MHZ;
		}
	} else {
		/* ADPLLM */
		if (data->m4x) {
...
static uint32_t clk_adpllm_hsdiv4_get_freq(struct clk *clkp)
{
	...
	if (pll_clk == NULL) {
		...
		ret = clk_adpllm_get_freq_internal(pll_clk, multiplier,
						   div_var);
	}
